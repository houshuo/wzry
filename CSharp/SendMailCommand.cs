...
[CheatCommand("通用/邮件/SendMail", "送心送钱邮件", 13), ArgumentDescription(0, typeof(GMMailAccessType), "属性类别", new object[] {  }), ArgumentDescription(2, typeof(string), "好友名称", new object[] { 0, "心" }), ArgumentDescription(1, typeof(int), "数值", new object[] {  })]
internal class SendMailCommand : CheatCommandNetworking
{
    ...
    {
        GMMailAccessType type = (GMMailAccessType) CheatCommandBase.StringToEnum(InArguments[0], typeof(GMMailAccessType));
        CheatCmdRef.stSendMail = new CSDT_CHEAT_SENDMAIL();
        switch (type)
        {
            case GMMailAccessType.金币:
                CheatCmdRef.stSendMail.bMailType = 1;
                CheatCmdRef.stSendMail.bMailsubType = 2;
                CheatCmdRef.stSendMail.bAccessCnt = 1;
                CheatCmdRef.stSendMail.astAccess = new COMDT_MAILACCESS[] { new COMDT_MAILACCESS() };
                ...
                CheatCmdRef.stSendMail.astAccess[0].stAccessInfo.stRongYu.dwRongYuPoint = CheatCommandBase.SmartConvert<int>(InArguments[1]);
                break;

            case GMMailAccessType.钻石:
                CheatCmdRef.stSendMail.bMailType = 1;
                CheatCmdRef.stSendMail.bMailsubType = 2;
                CheatCmdRef.stSendMail.bAccessCnt = 1;
                CheatCmdRef.stSendMail.astAccess = new COMDT_MAILACCESS[] { new COMDT_MAILACCESS() };
                ...
                CheatCmdRef.stSendMail.astAccess[0].stAccessInfo.stMoney.dwMoney = CheatCommandBase.SmartConvert<int>(InArguments[1]);
                break;

            case GMMailAccessType.心:
                ...
                CheatCmdRef.stSendMail.astAccess[0].stAccessInfo.stHeart.dwHeart = CheatCommandBase.SmartConvert<int>(InArguments[1]);
                break;
        }
