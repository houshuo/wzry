            ...
            if (this.m_dragonSpawnGroup != null)
            {
                ...
                else
                {
                    if (!this.m_isDragonOn)
                    {
                        ...
                        {
                            <>f__am$cache9 = src => (src.handle.TheActorMeta.ConfigId == Singleton<BattleLogic>.instance.DragonId) && (src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster);
                        }
