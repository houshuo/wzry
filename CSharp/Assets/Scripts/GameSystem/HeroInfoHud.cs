    ...
    public class HeroInfoHud
    {
        ...
        public void SetPickHero(HeroKDA hero)
        {
            if ((hero != null) && (this._pickedHero != hero))
            {
                ...
                this._headImg.SetSprite(string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_BustCircleSmall_Dir, CSkinInfo.GetHeroSkinPic((uint) this._pickedHero.actorHero.handle.TheActorMeta.ConfigId, 0)), Singleton<CBattleSystem>.GetInstance().WatchFormScript, true, false, false);
                if (this._showBuffDesc != null)
                {
        ...
        public void ValidateAd()
        {
            ...
            {
                this._adTxt.text = this._pickedHero.actorHero.handle.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_PHYATKPT].totalValue.ToString();
            }
        }
        ...
        public void ValidateAp()
        {
            if (this._pickedHero != null)
            {
                this._apTxt.text = this._pickedHero.actorHero.handle.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MGCATKPT].totalValue.ToString();
            }
        }
        ...
        public void ValidateEquip()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (!flag)
                {
                    image.SetSprite(string.Format("{0}EquipmentSpace", CUIUtility.s_Sprite_Dynamic_Talent_Dir), Singleton<CBattleSystem>.GetInstance().WatchFormScript, true, false, false);
                }
            }
        }
        ...
        public void ValidateMgcDef()
        {
            if (this._pickedHero != null)
            {
                this._mgcDefTxt.text = this._pickedHero.actorHero.handle.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MGCDEFPT].totalValue.ToString();
            }
        }
        ...
        public void ValidatePhyDef()
        {
            if (this._pickedHero != null)
            {
                this._phyDefTxt.text = this._pickedHero.actorHero.handle.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_PHYDEFPT].totalValue.ToString();
            }
        }
        ...
        public void ValidateReviceCD()
        {
            ...
            {
                this._reviveTxt.text = string.Format("{0}", Mathf.FloorToInt(this._pickedHero.actorHero.handle.ActorControl.ReviveCooldown * 0.001f));
