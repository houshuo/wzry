namespace Assets.Scripts.GameLogic
{
    ...
    public class CBattleValueAdjust
    {
        ...
        {
            this.UnInit();
            this.m_bSoulGrow = Singleton<BattleLogic>.GetInstance().m_GameInfo.gameContext.levelContext.IsSoulGrow();
        ...
        private void onActorStartFight(ref PoolObjHandle<ActorRoot> src)
        {
            if ((src != 0) && ((this.m_bSoulGrow && (src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)) && (src.handle.ValueComponent != null)))
            {
                ...
                if ((curLvelContext != null) && (curLvelContext.m_birthLevelConfig > 0))
                {
                    ...
                    src.handle.ValueComponent.SetHpAndEpToInitialValue(0x2710, 0x2710);
                }
                else
                {
