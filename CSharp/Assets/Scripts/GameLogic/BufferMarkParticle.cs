namespace Assets.Scripts.GameLogic
{
    ...
    internal struct BufferMarkParticle
    {
        ...
        private void CreateParticle(string _resName, ref PoolObjHandle<ActorRoot> _origin, ref PoolObjHandle<ActorRoot> _target)
        {
            if (_target != 0)
            {
                string prefabName = SkinResourceHelper.GetResourceName(ref _origin, _resName, true);
                this.particleObj = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD(prefabName, true, SceneObjType.ActionRes, _target.handle.gameObject.transform.position);
                if (this.particleObj == null)
                {
                    this.particleObj = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD(_resName, true, SceneObjType.ActionRes, _target.handle.gameObject.transform.position);
                }
                if (this.particleObj != null)
                {
                    ...
                    this.particleObj.transform.SetParent(_target.handle.gameObject.transform);
