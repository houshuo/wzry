namespace CSProtocol
{
    ...
    {
        public COMDT_STATISTIC_KEY_VALUE_INFO[] astKVDetail = new COMDT_STATISTIC_KEY_VALUE_INFO[50];
        ...
        public override TdrError.ErrorType pack(ref TdrWriteBuf destBuf, uint cutVer)
        {
            ...
            if (type == TdrError.ErrorType.TDR_NO_ERROR)
            {
                ...
                if (this.astKVDetail.Length < this.dwNum)
                {
                    return TdrError.ErrorType.TDR_ERR_VAR_ARRAY_CONFLICT;
                }
                for (int i = 0; i < this.dwNum; i++)
                {
                    type = this.astKVDetail[i].pack(ref destBuf, cutVer);
                    if (type != TdrError.ErrorType.TDR_NO_ERROR)
                    {
                        return type;
                    }
        ...
        public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
        {
            ...
            if (type == TdrError.ErrorType.TDR_NO_ERROR)
            {
                ...
                for (int i = 0; i < this.dwNum; i++)
                {
                    type = this.astKVDetail[i].unpack(ref srcBuf, cutVer);
                    if (type != TdrError.ErrorType.TDR_NO_ERROR)
                    {
                        return type;
                    }
