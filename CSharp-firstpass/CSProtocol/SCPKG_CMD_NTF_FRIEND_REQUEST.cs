    ...
    public class SCPKG_CMD_NTF_FRIEND_REQUEST : ProtocolObject
    {
        ...
        public override TdrError.ErrorType pack(ref TdrWriteBuf destBuf, uint cutVer)
        {
            ...
            if (type == TdrError.ErrorType.TDR_NO_ERROR)
            {
                ...
                int count = TdrTypeUtil.cstrlen(this.szVerificationInfo);
                if (count >= 0x80)
                {
                    return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
                }
                type = destBuf.writeCString(this.szVerificationInfo, count);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
        ...
        public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
        {
            ...
            if (type == TdrError.ErrorType.TDR_NO_ERROR)
            {
                ...
                if (dest > this.szVerificationInfo.GetLength(0))
                {
                    if (dest > LENGTH_szVerificationInfo)
                    {
                        return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
                    }
                    this.szVerificationInfo = new byte[dest];
                }
                if (1 > dest)
                {
                    return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_SMALL;
                }
                type = srcBuf.readCString(ref this.szVerificationInfo, (int) dest);
                if (type == TdrError.ErrorType.TDR_NO_ERROR)
                {
                    if (this.szVerificationInfo[((int) dest) - 1] != 0)
                    {
