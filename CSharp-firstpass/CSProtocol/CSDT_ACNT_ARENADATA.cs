namespace CSProtocol
{
    ...
    public class CSDT_ACNT_ARENADATA : ProtocolObject
    {
        ...
        public static readonly int CLASS_ID = 0x23e;
        ...
        public override void OnRelease()
        {
            ...
            this.dwNextRefreshLeftTime = 0;
            this.dwLeftFightCoolTime = 0;
        ...
        public override TdrError.ErrorType pack(ref TdrWriteBuf destBuf, uint cutVer)
        {
            ...
            type = destBuf.writeUInt32(this.dwTopRank);
            if (type == TdrError.ErrorType.TDR_NO_ERROR)
            {
                type = destBuf.writeUInt8(this.bChallengeLimitCnt);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = destBuf.writeUInt8(this.bBuyChallengeCnt);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = destBuf.writeUInt32(this.dwNextRefreshLeftTime);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = destBuf.writeUInt32(this.dwLeftFightCoolTime);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = destBuf.writeInt8(this.chAlreadyFightCnt);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
