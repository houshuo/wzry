namespace ResData
{
    ...
    public class ResDT_WealConInfo : IUnpackable, tsf4g_csharp_interface
    {
        ...
        public uint[] ReachConParam = new uint[4];
        ...
        public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
        {
            ...
            {
                type = srcBuf.readUInt32(ref this.dwConType);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = srcBuf.readUInt32(ref this.dwGoalValue);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                for (int i = 0; i < 4; i++)
                {
                    type = srcBuf.readUInt32(ref this.ReachConParam[i]);
                    if (type != TdrError.ErrorType.TDR_NO_ERROR)
                    {
                        return type;
                    }
                }
                type = srcBuf.readUInt32(ref this.dwFixedRewardID);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = srcBuf.readUInt32(ref this.dwLimitDays);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = srcBuf.readUInt32(ref this.dwLimitRewardID);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
                type = srcBuf.readUInt32(ref this.dwJumpEntry);
                if (type != TdrError.ErrorType.TDR_NO_ERROR)
                {
                    return type;
                }
